...
public class UserController {
    ...
    public String registration(Model model) {
        ...
        //if `password` length is length than equals to 2 return error string
        if(newUserPassword.length() <= 2) {
            return "passwordTypeError";
        }
        //used `regex` to validate if password has atleast 1 alphabet, 1 number & 1 special character
        if(!newUserPassword.matches("(?=.*[0-9]).*")) {
            return "passwordTypeError";
        }
        if(!newUserPassword.matches("(?=.*[a-z]).*")) {
            return "passwordTypeError";
        }
        if(!newUserPassword.matches("(?=.*[A-Z]).*")) {
            return "passwordTypeError";
        }
        if(!newUserPassword.matches("(?=.*[~!@#$%^&*()_-]).*")) {
            return "passwordTypeError";
        }
